...
function makeAPI() {
	...
	let dbFile = './db.db';
	let sqlFile = './init.sql';

	if (isDev) {
		dbFile = path.join(__dirname, '..', 'db.db');
		sqlFile = path.join(__dirname, '..', 'init.sql');
	}

	const db = makeDatabase(dbFile);
	const INIT_SQL = readFileSync(sqlFile, 'utf8');
	db.exec(INIT_SQL);
